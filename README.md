...
- **真实 target：Pi + Ark Agent Plan（#37）**：`harness: "pi"` 的 v1 config（`schema_version: 1`）由 `aco.pi_agent:PiAgent` 在 Harbor agent seam 执行，当前只接受精确的固定组合：`pi@0.84.1`（镜像 `aco-pi-agent:0.84.1` 由 node 固定 digest + python3 + 包版本预构建；执行管理器在拉起 supervisor 前自动预构建，也可用 `aco build-agent-image` 手动执行——Trial 启动路径绝不构建镜像、不运行 npm/apt、不访问 registry，只解析已构建镜像的不可变 digest，且 task.toml 以该 digest 钉死运行镜像）、provider `ark-agent-plan`、model `glm-5.3-flash`、thinking `max`、`provider_api_style: "openai-responses"`、`adapter_version: "0.1.0"`、`assistance_mode: "none"`、`credentials: ["ark-agent-plan-main"]`。运行要求：
