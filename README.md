...
- **真实 target：Pi + Ark Agent Plan（#37）**：`harness: "pi"` 的 v1 config（`schema_version: 1`）由 `aco.pi_agent:PiAgent` 在 Harbor agent seam 执行。首个组合是 `pi@0.84.1`（镜像 `aco-pi-agent:0.84.1` 由 node 固定 digest + python3 + 包版本预构建；操作员必须先执行 `aco build-agent-image`——Trial 启动路径绝不构建镜像、不运行 npm/apt、不访问 registry，只解析已构建镜像的不可变 digest，且 task.toml 以该 digest 钉死运行镜像）、provider `ark-agent-plan`、model `glm-5.3-flash`、thinking `max`、`provider_api_style: "openai-responses"`、`adapter_version: "0.1.1"`、`assistance_mode: "none"`、`credentials: ["ark-agent-plan-main"]`。运行要求：
  - 第二个受支持组合使用相同 Pi 镜像、provider 与凭证引用，model 为 `doubao-seed-2.0-mini`、thinking 为 `high`、adapter 版本为 `0.1.2`；旧 GLM 配置仍保留 `0.1.1`，两者的运行条件和结果序列分开。
...
`GET /v1/results`（可选过滤：`task_set`、`config`、`scorer`、`batch`，均 `name@version`；`view=raw|unified|default`）在服务端完成过滤与聚合，dashboard `/dashboard/results` 只渲染其返回值，统计公式不进前端：
